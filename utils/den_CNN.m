function [x_hat, z, g, v] = den_CNN(model, y, L)
% [x_hat, z, g, v] = den_CNN(model, x)
%
% INPUT
%   model : model CNN filters
%   y : input signal 
%
% OUTPUT
%   x_hat : output of CNN
...
%     dpt = length(h);
    ...
    % first layer
    z{1} = first_conv1d_layer(h{1, 1}, y);
    g{1} = z{1};
    v{1} = relu_layer(g{1});
    
    % hidden layers
    for i = 2:dpt-1
        [z{i}, g{i}] = conv1d_layer(v{i-1}, h{1, i});
        v{i} = relu_layer(g{i});
    end
    
    % final layers
    [z{dpt}, g{dpt}] = final_conv1d_layer(v{dpt-1}, h{1, dpt});
